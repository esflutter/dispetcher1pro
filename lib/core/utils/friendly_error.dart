...
String friendlyError(Object e, {String fallback = 'Не удалось выполнить. Попробуйте ещё раз.'}) {
  ...
  // Новый серверный код (миграция 118): отклик/принятие по услуге, чьи
  // документы ещё не одобрены. Появляется только в режиме документов-по-
  // услугам — про статус услуги, а не аккаунта.
  if (raw.contains('service_not_verified') &&
      SettingsService.instance.perServiceDocsCached) {
    return 'Документы по выбранной услуге ещё не одобрены. Проверьте статус в «Моих услугах».';
  }
  // Услугу удалили/заархивировали, пока пользователь был на её экране
  // (например, отправлял документы) — повторять бессмысленно.
  if (raw.contains('service_not_found')) {
    return 'Услуга больше не доступна (удалена или в архиве). Вернитесь в «Мои услуги».';
  }
