...
class ExecutorCardData {
  ...
  /// Подтягиваются из доступных услуг исполнителя (объединение, distinct,
  /// порядок первого появления). В бесплатном режиме доступ есть у всех
  /// услуг, включая старые записи с `isPaid=false`: они не должны исчезать
  /// из карточки и создавать впечатление скрытого платного ограничения.
  ...
  static List<String> _unionFromServices(
    ...
    for (final ServiceMock s in ServiceData.services) {
      if (!s.isPaid && !SettingsService.instance.freeModeCached) continue;
